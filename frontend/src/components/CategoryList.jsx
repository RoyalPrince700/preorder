...
const CategoryList = ({ activeCategory, onCategoryClick }) => {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      setLoading(true);
      try {
        const [allProductsRes, categoryRes] = await Promise.all([
          fetchAllProducts(),
          fetch(SummaryApi.categoryProduct.url, {
        ...
        const categorySamples = categoryData?.data || [];

        const countInCategory = (category) =>
          allProducts.filter((p) => p.category === category).length;

        const labelFor = (value) =>
          productCategory.find((c) => c.value === value)?.label ||
        ...
        const rows = [
          {
            ...
            image: allProducts[0]?.productImage?.[0],
            count: allProducts.length,
          },
          ...categorySamples.map((product) => ({
            key: product.category,
            name: labelFor(product.category),
            href: `/product-category?category=${product.category}`,
            image: product?.productImage?.[0],
            count: countInCategory(product.category),
          })),
        ];
  ...
  if (loading && categoryRows.length === 0) {
    return (
  ...
  return (
    <section className="w-full px-3 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-nowrap gap-3 overflow-x-auto pb-2 pt-2 scrollbar-none [scrollbar-width:none]">
          {categoryRows.map((row) => {
            ...
            const content = (
              <>
                <span
                  className={`text-xs font-black uppercase tracking-widest transition-colors sm:text-sm ${
